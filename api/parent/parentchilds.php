...
if($_SERVER['REQUEST_METHOD'] === 'GET')
{
   ...
   // $admin_id = (int)$admin_id;
    $query="SELECT students.id,attend,students.name AS name ,teachers.name AS teacher_name,  teachers.Alhalka_Number FROM students JOIN teachers ON students.teacher_id = teachers.id WHERE parent_id=?; ";
    
    $stm_students= mysqli_prepare($con,$query);
    $data=[];
    if($stm_students)
    {
        mysqli_stmt_bind_param($stm_students,'i',$parent_id);
